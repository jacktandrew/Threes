...
#{input.upcase}! Give me a break..."
  ...
  def get_first_computer_bet(p_num)  
    if @purse == 0
      puts "\n", " \s Looks like your all in, so we'll skip betting"
    elsif $purseP1 == 0 || $purseP2 == 0
      puts "\n", " \s Looks like your opponent is all in, so we'll skip betting"
...
I guess we'll just see where the dice land"
  ...
  def computer_call_or_raise(p_num)
    if $the_bet > 0
      @bet = $the_bet
    elsif $the_bet == 0
      @bet = 1
    end
    puts "\n", "So Mr. Computer you bet #{@bet}", "\n"
    @purse -= @bet
    $pot += @bet
    $the_bet = @bet - $the_bet
    puts "Player 1, Hit <ENTER> to let the computer take its turn"
    STDIN.gets
  end
 ...
 <> You've got #{@purse} left in your purse and the pot stands at #{$pot}"
  ...
  def computer_fold_or_call(p_num)
    if $tallyP1 < $tallyP2
      @bet = 1
    elsif $tallyP1 > $tallyP2
      @bet = 2
...
def vs_computer_game()
  ...
  while true
    ...
    break if $remainingP2 == 0 && $tallyP1 > $tallyP2
    @first_player.check_for_a_winner(1)
    @computer.computer_call_or_raise(0)  
    @first_player.fold_or_call(1)
  
    @computer.computer_roll(0) # roll > show > pick
    @computer.report_computer_choices(0) # report > get_first_bet
    break if $remainingP1 == 0 && $remainingP2 == 0
    break if $remainingP1 == 0 && $tallyP1 < $tallyP2
    break if $remainingP2 == 0 && $tallyP1 > $tallyP2
